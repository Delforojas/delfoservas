...
class BonosController extends AbstractController
{
    ...
    #[Route('/usuario/{id}/bono-activo', methods: ['GET'])]
public function bonoActivo(int $id, BonosRepository $repo): JsonResponse
{
    $bono = $repo->findBonoActivoByUsuario($id);

    if (!$bono) {
        return $this->json(['error' => 'Este usuario no tiene bono activo'], 404);
    }

    return $this->json($bono);
}
}